...
func NewBodyMapFrom(mapData interface{}) BodyMap {
	data := make(map[string]interface{})
	mapVal := reflect.ValueOf(mapData)
	if mapVal.Kind() == reflect.Map {
		keyVals := mapVal.MapKeys()
		for _, keyVal := range keyVals {
			key, err := acast.ToStringE(keyVal.Interface())
			if err != nil || key == "" {
				continue
			}
...
// 删除参数
func (bm BodyMap) Remove(key string) BodyMap {
	delete(bm, key)
	return bm
}

// 删除参数
func (bm BodyMap) Removes(keys ...string) BodyMap {
...
// 删除子参数
func (bm BodyMap) Delete(keys ...string) BodyMap {
	valMap := bm
