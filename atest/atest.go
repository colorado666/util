...
func AssertGT(value, expect interface{}) {
	...
	case reflect.String:
		passed = acast.ToString(value) > acast.ToString(expect)

	case reflect.Int, reflect.Int8, reflect.Int16, reflect.Int32, reflect.Int64:
		passed = acast.ToInt(value) > acast.ToInt(expect)

	case reflect.Uint, reflect.Uint8, reflect.Uint16, reflect.Uint32, reflect.Uint64:
		passed = acast.ToUint(value) > acast.ToUint(expect)

	case reflect.Float32, reflect.Float64:
		passed = acast.ToFloat64(value) > acast.ToFloat64(expect)
	}
	if !passed {
		panic(fmt.Sprintf(`[ASSERT] EXPECT %v > %v`, value, expect))
	}
...
func AssertIN(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		expectSlice := acast.ToIfaceSlice(expect)
		for _, v1 := range acast.ToIfaceSlice(value) {
			result := false
...
func AssertNI(value, expect interface{}) {
	...
	case reflect.Slice, reflect.Array:
		for _, v1 := range acast.ToIfaceSlice(value) {
			result := true
			for _, v2 := range acast.ToIfaceSlice(expect) {
				if v1 == v2 {
					result = false
					break
				}
		...
		if rvValue.Kind() == reflect.Map {
			if rvExpect.Len() == rvValue.Len() {
				...
				for _, key := range ksValue {
					mValue[acast.ToString(key.Interface())] = acast.ToString(rvValue.MapIndex(key).Interface())
				}
				for _, key := range ksExpect {
					mExpect[acast.ToString(key.Interface())] = acast.ToString(rvExpect.MapIndex(key).Interface())
				}
...
func getBacktrace(skip ...int) string {
	...
	index := 1
	from := 0
	// Ignore current atest lines and find the beginning index of caller file.
	for i := 0; i < 10; i++ {
		if _, file, _, ok := runtime.Caller(i); ok {
			if reg, _ := regexp.Compile(`atest\.go$`); !reg.MatchString(file) {
				from = i
				break
			}
		}
