...
const (
	...
	// 这里求最大值使用了位运算，-1 的二进制表示为 1 的补码，可以自己算算试试 -1 ^ (-1 << nodeBits) 这里是不是等于 1023
	...
	//起始时间 2020-03-03
	epoch int64 = 1583201149908 // 这个是我在写epoch这个变量时的时间戳(毫秒)
)
...
func (w *Worker) GetId() (int64, error) {
	...
	if now < w.timestamp {
		return 0, errors.New("Clock moved backwards.  Refusing to generate id for " + strconv.FormatInt(w.timestamp-now, 10) + " milliseconds")
	}
	if w.timestamp == now {
		w.number++
...
//解析生成的ID
func (w *Worker) Parse(id int64) (int64, int64, int64, error) {
	//先提取时间戳部分
	timestamp := (id & (-1 << timeShift)) >> timeShift

	//再提取workerId部分
	workerId := (id & (workerMax << workerShift)) >> workerShift

	//序号部分
	sequence := id & numberMax

	//解析错误
	if workerId != w.workerId || workerId > workerMax {
		return 0, 0, 0, fmt.Errorf("parse failed：invalid id, originWorkerId=%d\tparseWorkerId=%d\n",
			w.workerId, workerId)
	}
	if sequence < 0 || sequence > numberMax {
